...
class Boid:
    ...
    def _screen_wrap(self) -> None:
        if self.x < 0:
            self.x = config.WIDTH  # Re-enter from the right wall
        elif self.x > config.WIDTH:
            self.x = 0  # Re-enter from the left wall

        if self.y < 0:
            self.y = config.HEIGHT  # Re-enter from the bottom wall
        elif self.y > config.HEIGHT:
            self.y = 0  # Re-enter from the top wall
